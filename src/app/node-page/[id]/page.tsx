...
export default function NodeDetailPage() {
  ...
  if (loading) {
    return (
      <main className="min-h-screen pt-20 md:pt-40">
  ...
  if (!node) {
    return (
      <main className="min-h-screen pt-20 md:pt-40">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-black/80 backdrop-blur-md rounded-xl border border-white/10 overflow-hidden shadow-xl p-4 md:p-8 text-center">
            <h1 className="text-xl md:text-2xl font-bold text-white mb-4">Node Not Found</h1>
  ...
  return (
    <main className="min-h-screen pt-20 md:pt-40">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 mt-6 md:mt-16">
        <div className="mb-6 md:mb-8">
          <Link 
            href="/node-page" 
            className="inline-flex items-center px-3 py-1.5 md:px-4 md:py-2 bg-black/40 hover:bg-black/60 text-emerald-400 hover:text-emerald-300 rounded-lg border border-emerald-500/30 transition-all duration-200 group shadow-md text-sm md:text-base"
          >
            <svg 
        ...
        <div className="bg-black/80 backdrop-blur-md rounded-xl border border-white/10 overflow-hidden shadow-xl p-4 md:p-8">
          <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-4 md:mb-8">
            <div className="mb-4 md:mb-0">
              <h1 className="text-xl md:text-2xl font-bold text-white">{node.nodeName}</h1>
              <p className="text-white/50 text-sm">ID: {node.nodeID}</p>
            </div>
          ...
          <div className="bg-black/50 rounded-lg border border-white/10 p-4 md:p-6 mb-4 md:mb-6">
            <h2 className="text-base md:text-lg font-medium text-white mb-2 md:mb-4">Node Details</h2>
          ...
          <div className="bg-black/50 rounded-lg border border-white/10 p-4 md:p-6 mb-4 md:mb-6">
            <h2 className="text-base md:text-lg font-medium text-white mb-2 md:mb-4">Dataset</h2>
            <div className="grid grid-cols-1 gap-4">
              <div 
                onClick={() => window.location.href = `/plant-data/${sensorData.plantID}`} 
                className="bg-black/40 border border-emerald-500/30 rounded-lg p-3 md:p-4 flex flex-col items-center cursor-pointer hover:bg-emerald-900/20 hover:border-emerald-500/50 transition-all duration-300 transform hover:-translate-y-1 hover:shadow-lg hover:shadow-emerald-500/20"
              >
                
                <div className="text-white font-medium text-base md:text-lg">{sensorData.plantName}</div>
      ...
      {showPlantModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
          <div className="bg-black/90 backdrop-blur-md rounded-xl border border-white/10 p-4 md:p-8 w-full max-w-md">
            <h3 className="text-lg md:text-xl font-bold text-white mb-4">Create New Plant</h3>
            <form onSubmit={handleCreatePlant}>
              <div className="mb-6">
                <label htmlFor="plantName" className="block text-white/70 mb-2 text-sm md:text-base">Plant Name</label>
                <input
                  type="text"
                  id="plantName"
                  value={plantName}
                  onChange={(e) => setPlantName(e.target.value)}
                  className="w-full bg-black/50 border border-white/20 rounded-lg px-3 py-2 md:px-4 md:py-3 text-white focus:outline-none focus:border-emerald-500"
              ...
              <div className="flex justify-end space-x-3">
                <button
                  ...
                  className="px-3 py-1.5 md:px-4 md:py-2 border border-white/20 text-white/70 rounded-lg hover:bg-white/10 transition-all duration-200 text-sm md:text-base"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-3 py-1.5 md:px-4 md:py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg transition-all duration-200 text-sm md:text-base"
                >
                  Create
                </button>
              </div>
